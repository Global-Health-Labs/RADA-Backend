import { pgTable, uuid, varchar, boolean, timestamp, integer, text, real } from 'drizzle-orm/pg-core';
...
export const masterMixes = pgTable('master_mix', {
  ...
  experimentalPlanId: uuid('experimental_plan_id').references(() => experimentalPlans.id),
...
export const documents = pgTable('document', {
  id: uuid('id').defaultRandom().primaryKey(),
  experimentPlanId: uuid('experiment_plan_id').references(() => experimentalPlans.id),
...
export const masterMixRecipesRelations = relations(masterMixRecipes, ({ one }) => ({
...
export const experimentalPlansRelations = relations(experimentalPlans, ({ one, many }) => ({
  owner: one(users, {
